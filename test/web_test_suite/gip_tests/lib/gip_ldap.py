...
def query_bdii(cp, query="(objectClass=GlueCE)", base="o=grid"):
    ...
    """
    bdii_addr = cp.get("gip", "bdii_addr")
    bdii_port = cp.get("gip", "bdii_port")
    endpoint = "ldap://" + bdii_addr + ":" + bdii_port
    ...
    info['base'] = base
    if query == '':
        cmd = "ldapsearch -h %(hostname)s -p %(port)s -xLLL -b %(base)s" % info
    else:
        cmd = "ldapsearch -h %(hostname)s -p %(port)s -xLLL -b %(base)s '%(query)s'" % info
...
def getSiteList(cp):
    ...
    """
    fp = query_bdii(cp, query="(&(objectClass=GlueTop)(!(objectClass=GlueSchemaVersion)))")
